      ...
      static struct option long_options[] =
        {
          ...
          {"numRoots",            required_argument, 0, 'n'},
          {"moints_filename",     required_argument, 0, 'm'},
          {"configs_filename",    required_argument, 0, 'c'},
          {"vector_filename",     required_argument, 0, 'v'},
          {"vector_restart",      no_argument,       0, 'r'},
          {"hamiltonian_restart", no_argument,       0, 'H'},
          {"subspace_min_mult",   required_argument, 0, 'i'},
          {"subspace_max_mult",   required_argument, 0, 'x'},
          {"subspace_iterations", required_argument, 0, 'I'},
          {"coarse_rtol",         required_argument, 0, 'C'},
          {"final_rtol",          required_argument, 0, 'F'},
          {"maxsparse",           required_argument, 0, 's'},
          {"showevals",           no_argument,       0, 'S'},
          {"version",             no_argument,       0, 'V'},
          {"help",                no_argument,       0, 'h'},
          {0, 0, 0, 0}
        };
      int option_index = 0;
      c = getopt_long (argc, argv, "n:dm:c:v:ri:a:I:C:F:hHVS",
...
*/
   ...
   */  
   ...
   if ( construct_hamiltonian == CONSTRUCT ) {
     ...
     readSparse = SpecifyDRA(construct_hamiltonian, d_cimat,d_icicol,d_number,d_diag_sef);  
   ...
   */
   ...
   if( construct_hamiltonian == RESTART && maxsparse < readSparse ) {
     GCOUT << "On Hamiltonian READ a different value for sparsity was identified: Changing to match " << readSparse << endl;
...
// NOT DOING THIS ADDS A FW H*v TO THE diagonalization startup
 ...
 const int maxsimultanious = 2; // Do max at a time as a rolling solver
 GCOUT << "Experimental (partial) BLOCK Davidson method: maxsimultanious is "<< maxsimultanious << endl;
 int numUpdates = vector_set.CurrentExistingRoots();

 vector<int> listRnorms; // Maintains the GA position of the rnorm object to use for preconditioning
...
//      curroot = itest; //So we could make this an array of rnorms to keep
      listRnorms.push_back( itest ); // do not subtract one 
      if (listRnorms.size() >= maxsimultanious ) {
          curroot = listRnorms[0]; // Lowest level of the current group
          break;
      }
    }
    ++itest;
    ...
    if( converged == false ) {
      tempTime = psociTime();
      for(rit=listRnorms.begin(); rit != listRnorms.end(); ++rit ){
        double shift = evals[ (*rit)-1 ]; // Try to update vector == curroot 
        int nump = vector_set.preconditionAndNewVectorIncore((*rit), shift, g_rnorms, hamilton.fetchDIAG_SEF() );
      ...
      numUpdates = listRnorms.size();// Number needing processing
    }
...
// Dump current full set of vectors to disk for possible restart or post processing
  ...
  if ( GA::nodeid()== 0 ) {
    ...
    for(int i=0; i< vector_set.CurrentNumSoughtRoots(); ++i ) {
      GCOUT << "Eigenvalue " << i << " Total energy (au) " << setprecision(6) << evals[i]+fcore <<  " Electronic (au) " << evals[i] << " rnorm " << setprecision(8) << rnorms[i] << endl;
    }
  }
