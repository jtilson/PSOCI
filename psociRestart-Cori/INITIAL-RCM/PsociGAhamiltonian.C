...
* http://www.renci.org/resources/open-source-software-license
 ...
 History:
              ...
              Added a new preprocessing flag  VISUALIZEHAM to dump non-zero values to local files
...
/* experimental: modified to also create the vector m,ask data objects
*/
...
// use by the matrix-vector product schemes
// For now we assume no more than maxsparse elements per rank (row)
...
/* Experimental: add creation of vector mark objects */
				    ...
				    vector<pair<long,long> > & stats, vector<vector<int> > & matrix )
{
  ...
  for(int i=0; i< nsefi; ++i ) {
	...
	++number[i]; 
        ++numElems;
// potential messup for isef scrambled
...
//cout << "fetch chunk " << v_lo[0]<<" "<<v_hi[0]<<" "<<testVector.size() <<" "<<chunk_width << endl;
...
// Need to fix and utilize this!
         //if ( GA::nodeid() == 0 )  cout << "manual remap of loop " << endl;

          int chi = l_maxsefs-size -1;
         if ( GA::nodeid() == 0 )  cout << "manual remap of loop last chi is " << chi << endl;
...
//cout << "fetch block " << buf_index << " " << i << " "<<indexnewarray << endl;
...
//      const int maxelements = 1*1024*1024; // limits scatter stack to something like 16 MB

// lots of small updates instead
        const int maxelements = 10;

       if  ( (indexnewarray >= MAX_AGGREGATE-1 ) || estimate_elements > maxelements || i >= chi) { // 100000 value should tie to GA stack

     //    if ( (indexnewarray >= MAX_AGGREGATE-1 ) || estimate_elements > maxelements ) {

//cout << "packData " << indexnewarray<<" "<<MAX_AGGREGATE<<" "<<estimate_elements<<" "<<maxelements<<" "<<i<<" "<<chi <<endl;
//cout << "send in data " << bufVnumber[0] <<" "<< indexnewarray<<" "<<transpose << endl;
...
//cout << " INside the Scatter loops " << newrealnumber << endl;
                     ++numscatters;
#ifdef TACC
#ifdef GAEXP 
      indexToInteger( indexarray, indexarrayInteger );
      ...
      //cout << "sctater " << newrealnumber<< " " << i << " to "<< indexarray[0]<<" "<<indexarray[1] << endl;

        int c_flag=0;
        indexToInteger( indexarray, indexarrayInteger );
        pnga_scatter_acc( g_Hv->handle(), (void *)&dataarray[0], (Integer *)&indexarrayInteger[0], c_flag, newrealnumber, &alpha); // low level call
...
//      NGA_Scatter_acc_flat(g_Hv->handle(), (void *)&dataarray[0], (int*)&indexarray[0], newrealnumber, &alpha);
    ...
    //  GA::sync();
      tempTime2 = psociTime() - tempfull;
      cout << GA::nodeid() << " NEW ACCUMULATE SCAT: numscatters fetchLocal cimatTime getV accTime compTime1 comptTime2 tempTime2 scatTime packTime "<< numscatters << " "<<fetchLocal<<" " << cimatTime<<" "<<  getVTime<<" "<<accTime << " "<<compTime1<<" "<<compTime2<<" "<< tempTime2 << " " <<  scatTime<<" "<<packTime <<" " << endl;
...
// New scatter based stuff
  ...
  cout << GA::nodeid() << " my vecot rrange is " << v_lo[0]<<" "<<v_hi[0] << endl;
...
// Perhaps we should wait ?
        ...
        //cout << GA::nodeid() << " after gather chunk " << endl;
        numgets++;
...
//cout << " get Hblock " << GA::nodeid() << " "<<i<<" "<<buf_index << endl;
...
//cout << " get first block " << GA::nodeid() << " "<<  number<<" "<<i<<endl;
	      if (!ch_end-1 < start_rng  || !ch_start > end_rng ) { //none within the range
		for(int j=0; j< number; ++j ) {
		  jhi = local_icicol[buf_index][j];
		  if ( jhi >= ch_start && jhi < ch_end ) {
		    dtemp += local_cimat[buf_index][ j ] * testVector[ jhi - ch_start]; 
//cout << "dtemp " << dtemp<<" "<<jhi<<" "<<local_cimat[buf_index][ j ]<<" "<<testVector[ jhi - ch_start]<<endl;
                  }
		}
	      ...
	      if ( i >= ch_start && i < ch_end ) {
                temp = psociTime();
//cout << " get nest block " << GA::nodeid() << " "<<  number<<" "<<i<<endl;
		double testVec = testVector[ i - ch_start ]; 
...
//cout << "HV off " << jhi<<" "<<testVec<<" "<<local_cimat[buf_index][ j ]<<endl;
                    }
		}
		compTime2+=psociTime() - temp;
	      } // ch_start screen
...
//cout << " accujulate block " << GA::nodeid() << " "<<  number<<" "<<i<<endl;
...
// doesn't do anything
    ...
    double timevectorgets=0.0;
    int numvectorgets=0;
    int totalElems=0;

    int my_vector_low = v_lo[0];
    int my_vector_hi = v_hi[0]; // this does not need subtraction by 1
    int vec_width = my_vector_hi - my_vector_low + 1; // inclusive
    double tempVec=psociTime();
    testVector.resize( vec_width );


     int checkLoops=0;

    //  cout <<" vector ranges "<<v_lo[0]<<" "<<v_hi[0]<<" "<<vec_width<<" "<<testVector.size() << endl;
/*
      if ( GA::nodeid() == 0 ) {
         cout << "maxspatials is " << l_maxspatials << endl;
         for (int j=0; j< l_maxspatials; j++ ) {
         cout << "spatial uinsertion point is " << j <<" "<<l_nsef[j]<<endl;
         }
      }
     GA::sync();
*/
//DO a simple thing. Simply sweep through l_nsef and find matches.

      int iconflow=-1;
      for( int it=0; it<l_maxspatials; it++){
          int itemp = l_nsef[ it ];
          //cout << GA::nodeid() << " test " << itemp <<" "<<it <<" v_lo " << v_lo[0] << endl;
          if ( itemp == v_lo[0] ) {
             iconflow = it;
             //cout << GA::nodeid() <<  " GOT iconflow " << iconflow <<" "<<it<<" itrem was " << itemp<<" "<<v_lo[0] << endl;
             break;
          }
     }
     //if ( iconflow < 0 ) GA::error("Never found first  iconflow ",1);
    
// Now how to find iconfhi?

      int iconfhi=-1;
      for( int it=0; it<l_maxspatials; it++){
          int itemp = l_nsef[ it ]; // recall l_nsef are entry points
          //cout <<GA::nodeid() << " TEST HI "<<it<<" "<<itemp<<" "<<v_hi[0]<<endl;
          if ( itemp > v_hi[0] ) { // or is it >=? fiund the lowest upper bound
             iconfhi = it-1;
             //cout << GA::nodeid() <<  " 2nd GOT iconfhi " << iconfhi <<" "<<it<<" itrem was " << itemp<<" "<<v_hi[0] << endl;
             break;
          }
     }
     //if ( iconfhi < 0 ) GA::error("Never found iconfhi ",1);
     if ( iconfhi < 0 )  iconfhi = l_maxspatials-1;
     cout << GA::nodeid() << " FULL iconflow is " << iconflow << " "<< " iconfhi is " << iconfhi << " " << l_maxspatials << endl;

        int didnum =0, nodidnum =0;
...
//      int vec_width = my_vector_hi - my_vector_low + 1; // inclusive
      ...
      hi[1]=iroot;
      int n=1;
      if ( hi[0] - lo[0] +1 > testVector.size() ) {
          cout << "test vector path too small" << testVector.size() <<endl;
          GA::error("test vector path too small",testVector.size());
      }


// Grab and process all vectors at once
      g_v->get( lo, hi, &testVector[0], &n ); 
      numvectorgets++;
      timevectorgets += psociTime() - tempVec;


// Now limit J loops to between these

      // What possible Jconfs would apply to me based on this Vector ?
      // Now loop over all H entries - ( we will need some kind of filter here to be viable but for now do them all)

        checkLoops=0;

      // We need extrordinary filtering to make progress in this method
        for(int iconf = 0; iconf < l_maxspatials; ++iconf ) { // must loop over the entires H matrix BUT onlyaccumulate ove the pathcv Vi
...
// Can fetch instead of compute also
        temp2 = psociTime();
        l_deters->computeConfigsGA( iconf+1, imap, fetchi ); // resize imap then computeConfigs
        //l_deters->fetchAndUnpackDeterminantData( iconf+1, info1, fetchi );

        computeConfsTime += psociTime() - temp2;
        ++totalFetches;

// For the given iconf, find all jconfs, that are possible to interact based on DELTA(numopens).
        temp2 = psociTime();
        int iphase = rep_phases->at(fetchi.index-1);
        vector<int> jphases;
        int numJs = assemblePhasePossibles(iconf, iphase, jphases, rep_phases );
        phaseblock += jphases.size();
        cout << "First filter: assemblePhasePossibles sinple > 4 test returned " << numJs << " out of a total of " << iconf+1 <<" checked " << endl;
        assemTime += psociTime() - temp2;

// prune list of Jconfs to those only within the local testVector path range
        int prunetemp =  l_deters->pruneListVectorPatch(iconflow, iconfhi+1, jphases ); // INCLUDISVE
        prunerun += prunetemp;
        cout << "new second filter: pruneListVectorPatch " << prunetemp << " out of " << iconf+1 << endl;


// Now for the given set of Jconfs,  do a deeper comparison based on orb occs.
        temp2 = psociTime();
        vector<int> list;
        int cruntemp =  l_deters->compareAllOrbMapMatrixVectorProducts(iconf+1, imap, jphases, list );
        countrun += cruntemp;
        jphases.clear();
        cout << "new thirdfilter: compareAllOrbMapMatrixVectorProducts orbmap comparison " << cruntemp <<" list size is " << list.size() <<  " out of " << iconf+1 << endl;

        excitblock += list.size();
        compareTime += psociTime() - temp2;

// Don't by the memory cost without reason
          int width = fetchi.nsefi;
          //if ( list.size() > 0  && cimat.size() != width ) {
          ...
          totalElems = 0; // flush after every iconf
          vector<int>::iterator jit;
          for(jit = list.begin(); jit != list.end(); ++jit ) // jit are CONFs not SEFs
            {
              checkLoops++;
              int jconf = (*jit);
              vector<COMPRESS_SIZE> jmap( l_nbf+2  );
...
// See if any values exists in NUMBER
            int sumnum =0;
            ...
            if ( totalElems > 0 ) { // this should be uncommon as it is for the entire jconf
                  // cout << "Got values for M*V: nsefi should be  "<<fetchi.nsefi << endl;
                  temp2 = psociTime();
                  double Vextra=-1; // dummy for now
                  estimate_elements = matrixVectorUpdate(g_v, iroot, iconf, Vextra,v_lo, v_hi, indexnewarray, fetchi.nsefi, bufVnumber, bufVIndex, bufV2, icicol, cimat, number, testVector );
                  updateTime += psociTime() - temp2;
                  // cout << "estimate_elements = " << estimate_elements << endl;
            }

            // cout << GA::nodeid()  << " totalElems before SCATTER is " << totalElems << endl;
         ...
         //     cout << GA::nodeid() <<" sumnum was zero for iconf = " << iconf <<" numTest is " << testNumTest << endl;
             nodidnum ++;
            }
       // cout << GA::nodeid() << " DO next I " << endl;
...
//g_Hv->print();
        ...
        cout << GA::nodeid()<< " didnum nodidnum " << didnum << " " << nodidnum << endl; 

      //if ( GA::nodeid() == 0 ) {
        cout << GA::nodeid() << " checkLoops " << checkLoops<<endl;
	cout << GA::nodeid() << " sociTime is " << sociTime<<endl;
...
/* NON SPLIT version of the GATHER SCATTER methods
   compile code as -DGATHERHV
*/
...
if ( GA::nodeid()==0) cout << "matrixVectorProductsIncoreGatherScatterNoVectorGet" << endl;
...
// multimaps MUST NOT SPAN multiple ivecs
  ...
  cout << "MAX_DEFLATE_ELEMENTS " << MAX_DEFLATE_ELEMENTS<<" MAX_AGGREGATE" << MAX_AGGREGATE  << endl;
  cout << "cimat_luist size is "<< cimat_list.size() << endl;
...
//cout << "JEFF cimat list size " << cimat_list.size() << endl;
 ...
 //         if ( GA::nodeid() == 0) cout << "node zero list:cimat_list is " << it<<" size is " << cimat_list.size() << endl;
	  ...
	  for(int j=0; j< max_num; ++j) {
	    newlist.push_back( cimat_list[ it+j ]);
//if ( GA::nodeid() == 0)  cout << "test newlist " << cimat_list[ it+j ] << endl;
	  }
...
/* fetch set of vectors as well */
	  ...
	  for(int it2=0; it2< max_num; ++it2) { // extra loop
///cout << "compute loop " << endl;
//cout << "its " << it2<<" " << newlist.size() << "newlist val is " << newlist[ it2 ] << endl;
	    int i = newlist[ it2 ]; 
	      ++buf_index; // niot needed use it2
	      
//cout << "buf index " << buf_index<<" local size " << local_number.size() << endl;

	      number = local_number[buf_index];
//cout << "local numval is " << local_number[buf_index] << endl;
//cout << "num is " << number<<" bufV size is " << bufV.size()<< endl;
//              bufV.resize(number);
...
//cout << "At the deflate " << endl;
//Sort of per elements per row
...
int PsociGAhamiltonian::matrixVectorProductsIncoreGatherScatterNoVectorGetDecember( int start, int end,  GA::GlobalArray * g_v, int * v_dims, GA::GlobalArray * g_Hv, 
								 int * hv_dims, int * v_lo, int * v_hi )
{
#ifndef SUPPLEMENTAL
  GA::error(" Must compile code with -DSUPPLEMENTAL ",-1);
...
if ( GA::nodeid()==0) cout << "matrixVectorProductsIncoreGatherScatterNoVectorGetDecember" << endl;
...
// we could simply use bufV2[0] !!

  vector<double> bufV( max(l_maxsparse,l_maxsparse_supp), 0.0  );
  vector<int> bufindex( max(l_maxsparse,l_maxsparse_supp), 0  );
  ...
  double alpha = 1.0;
  double  fetchLocal=0.0, listTime=0.0,compTime1=0.0, compTime2=0.0, scatTime=0.0, getVTime=0.0, deflateTime=0.0;
  double deflateListTime=0.0, deflateVecTime=0.0;
...
// multimaps MUST NOT SPAN multiple ivecs
// Note also, the more we accumulate before deflation the more we may get partial summation
// especially for contiguous i .
...
//            for (int i=my_rank; i< l_maxsefs; i+=my_size ) { 
            for(int i=clow; i<=chi; ++i ) { //data parallel across maxsef
              ...
              const int buf_index=0; // only grab one row let I drive locality or not
...
// Get gather vector pieces for this row based on local_number[] which is a global vatriable (yuk) : only grabs what J's for the list of Is on newlist
// someting is wrong here - fix me - Nope its OKAY just horribly ugly and opaque in usage of local_number.
           
          temptime=psociTime();
          vector<int> newlist(1, i); // a single component == i
          int num = gatherVectorSet(g_v, ivec, newlist, vectors, dataarray, indexarray);
          numvecgets++;
          vectorTime += psociTime() - temptime;

//cout << "Size of return vector (num Js) i s " << vectors.size() << endl;

          int vecindex = -1; // index Js list inside of vectors[]
	      dtemp=0.0;
...
//cout << "loop one " << endl;
		...
		for(int j=0; j< number; ++j ) {
		  ...
		  jhi = local_icicol[buf_index][j];
              ...
              bufV[ number-1 ] = dtemp;
              bufindex[ number-1 ] = local_icicol[buf_index][ number-1  ];
	      compTime1 += psociTime() - temp;
		...
		for(int j=0; j< number-1; ++j ) {
		  int jhi = local_icicol[buf_index][j];
		  bufV[ j ] = local_cimat[buf_index][ j ] * testVec; //NO SUMMATION REQUIRED
                  bufindex[ j ] = local_icicol[buf_index][ j ];
		}
   ...
   bool transpose=false;
#endif
                  newnumber = packDataNoZeros1D(transpose, ivec, number, bufindex, bufV, indexarray, dataarray );
                  numatemptdeflates++;
		  ...
		  if ( newnumber > 0 ) {
		    ...
		    NGA_Scatter_acc_flat(g_Hv->handle(), (void *)&dataarray[0], (int*)&indexarray[0], newnumber, &alpha);
/*
cout << "pnga_scatter " <<newnumber <<  endl;
                    int c_flag=1;
                    pnga_scatter_acc( g_Hv->handle(), (void *)&dataarray[0], (int*)&indexarray[0], c_flag, newnumber, &alpha); // low level call
*/
...
//get straglers
	  ...
	  ++hvcount;
        ...
        allTime = psociTime()- temp4;
	cout << GA::nodeid() << "december: alltim vectorT scatTime comp1 comp2 deflate  numgetvecs numdeflates numatemptdeflates SUM "<< allTime<<" "<<vectorTime<<" "<<scatTime<<" "<<compTime1 <<" "<<compTime2 << " "<<deflateTime<<" "<<numvecgets<<" "<<numdeflates <<" "<< numatemptdeflates << " " <<vectorTime+scatTime+compTime1+compTime2+deflateTime << endl;
	
	GA::sync();
	return(hvcount);
      }

/* New method. In this case we have previously build a matrix named g_rank, g_rank_num which predetermined for my local array patch which I might
   possibly interact.
*/

/* currently only wortks on a single core */


int PsociGAhamiltonian::matrixVectorProductsIncoreGatherScatterLocalVectorGetMarch( int start, int end,  GA::GlobalArray * g_v, int * v_dims, GA::GlobalArray * g_Hv, 
								 int * hv_dims, int * v_lo, int * v_hi )
{
#ifndef SUPPLEMENTAL
  GA::error(" Must compile code with -DSUPPLEMENTAL ",-1);
...
// we could simply use bufV2[0] !!
  ...
  vector<double> testVector( width );// Vector buffer for individual H*v
...
// fetched local vector patch
     tempTime2=psociTime();
     zeroVector( testVector );
     n=1;
     g_v->get( lo, hi, &testVector[0], &n ); // we are only fetching the local piece into a full size space
     getTime += psociTime() - tempTime2;
     cout << " testvector size is " << testVector.size() << endl;
...
//            for (int i=my_rank; i< l_maxsefs; i+=my_size ) { 
// Data suggest we might only need to loop over vector range
// dont worry about performance issues yet

//           cout << "Ham range " << clow <<" "<< chi << endl;
//           for(int i=clow; i<=chi; ++i ) { //data parallel across maxsef

             vector<int> mask_list; 
             int getlist = fetchMaskList( my_rank, mask_list );
             cout << "Use MASK for list " << endl;

             cout << my_rank << " fetch I list of size " << getlist << endl;
             for(int itemp=0; itemp< mask_list.size(); ++itemp) {
                int i = mask_list[ itemp ];
		...
		for(int j=0; j< number; ++j ) {
		  ...
		  jhi = local_icicol[buf_index][j];
                    if ( jhi <=vhi && jhi >=vlow ) {
                      dtemp += local_cimat[buf_index][ j ] * testVector[jhi];
                    }
		}
                bufV[ number-1 ] = dtemp;
                bufindex[ number-1 ] = local_icicol[buf_index][ number-1  ];
	        ...
	        temp = psociTime();
                //double testVec = testVector[number-1]; // same as ending of last loop 
		...
		for(int j=0; j< number-1; ++j ) {
		  ...
		  if ( newnumber > 0 ) {
//cout <<" scatter num is " << newnumber << endl;
		    temp2=psociTime();
		    NGA_Scatter_acc_flat(g_Hv->handle(), (void *)&dataarray[0], (int*)&indexarray[0], newnumber, &alpha);
		    scatTime += psociTime() - temp2;
		    ++numdeflates;
...
//get straglers
	  ...
	  ++hvcount;
        ...
        allTime = psociTime()- temp4;
	cout << GA::nodeid() << "march: alltim vectorT scatTime comp1 comp2 deflate  numgetvecs numdeflates numatemptdeflates SUM "<< allTime<<" "<<vectorTime<<" "<<scatTime<<" "<<compTime1 <<" "<<compTime2 << " "<<deflateTime<<" "<<numvecgets<<" "<<numdeflates <<" "<< numatemptdeflates << " " <<vectorTime+scatTime+compTime1+compTime2+deflateTime << endl;
	
	GA::sync();
	return(hvcount);
      }
      
/* Attempt a new method that GATHERS vector data but loops ovewr J chunks.
   DECEMBER test 
*/
int PsociGAhamiltonian::matrixVectorProductsSplitAccumChunkGatherVector( int start, int end,  GA::GlobalArray * g_v, int * v_dims, GA::GlobalArray * g_Hv, 
								 int * hv_dims, int * v_lo, int * v_hi )
{
#ifndef SUPPLEMENTAL
  GA::error(" Must compile code with -DSUPPLEMENTAL ",-1);
  ...
  if ( GA::nodeid()==0) cout << "matrixVectorProductsSplitAccumChunkGatherVector" << endl;
  ...
  */
  ...
  int jchunk_hi[2];
   ...
   if ( GA::nodeid() == 0 ) cout << "highly experimental gather vector loop accumulate " << endl;

  int chunk_width;
  (l_maxsefs % nchunk==0)? chunk_width = l_maxsefs / nchunk: chunk_width = 1 + (l_maxsefs / nchunk);
  if ( GA::nodeid() == 0 ) cout << "NEW DEC: exp VECTOR get CHUNK is " << nchunk << " " << chunk_width << " "<<MAX_AGGREGATE<<endl;
...
#ifdef MATRIXBALANCE
  GA::error(" Must NOT define -DMATRIXBALANCE to use :matrixVectorProductsSplitAccumChunkGatherVecto",1); 
#endif

#ifdef SQUAREHAMILTONIAN
  GA::error(" Must NOT define -DSQUAREHAMILTONIAN to use :matrixVectorProductsSplitAccumChunkGatherVecto",1);
#endif

#ifdef STATICEVEN
 if ( GA::nodeid() == 0 ) cout << "STATIC EVEN is use " << endl;
#endif
  ...
  int numaccs=0;
     ...
     if ( bufV.size() != chunk_width ) bufV.resize( chunk_width );
...
//cout << GA::nodeid() << "LOCA H range for ME is " << clow<<" "<< chi<< endl;
...
//      cout << "jchunk_width > bufV.size() "<< bufV.size()<<" "<<jchunk_width  << endl;
//      if ( jchunk_width > bufV.size() ) GA::error(" jchunk_width > bufV.size()",bufV.size());

      zeroVector( bufV, jchunk_width ); 
...
#ifdef STATICEVEN
     for (int i=my_rank; i< l_maxsefs; i+=my_size ) {  // this one works fine
     // broken  for( int i = l_maxspatials-1-my_rank; i >= 0; i-=my_size ) {
#else

cout << "TOTAL local range for ME is " << chi-clow<< endl;
      for(int i=clow; i<=chi; ++i ) { //data parallel across maxsef

#endif

//cout << "I am " << GA::nodeid()<<" "<<i<<endl;
...
//  Get hamiltonian ROW
     ...
     end_rng=local_icicol[buf_index][local_number[buf_index]-1];
     number = local_number[buf_index];
     cimatTime += psociTime() - temp;

     temptime=psociTime();
     vector<int> newlist(1, i); // a single component == i

//   cout <<"Gather " << number<<" "<<ivec<<" "<<vectors.size() << " "<<dataarray.size()<<" "<<indexarray.size() << endl;
...
// Now do a jchunk loop. PLUS we can check vector to see if any I's exist

//cout << "process work " << i<<" "<<jch_start<<" "<<jch_end << endl;

//      if ( vectors.size() < number ) GA::error(" FIRST vector too smal " ,vectors.size());

       temp = psociTime();
       if ( i >= jch_start && i < jch_end ) {
          for(int j=0; j< number; ++j ) {
            jhi = local_icicol[buf_index][j];
            dtemp += local_cimat[buf_index][ j ] * vectors[ j ] ;
           }
         bufV[ i-jch_start ] += dtemp; // 
        }// i > jch
        compTime1+=psociTime() - temp;

//needs ot be fixed below here isn;t this just jmax?

       temp = psociTime();
       double testVec = vectors[ number-1 ];
       for(int j=0; j< number-1; ++j ) {
          int jhi = local_icicol[buf_index][j];
          if ( jhi >= jch_start && jhi < jch_end ) {
          bufV[ jhi-jch_start ] += local_cimat[buf_index][ j ] * testVec;
...
// probably okay below here 
	...
	cout << GA::nodeid() << "NEW gather Vec PUT HVEC: ciTime vectorT comp1 comp2 bufV num "<< allTime<<" "<<" "<< cimatTime<<" "<<vectorTime<<" "<<compTime1 <<" "<<compTime2 << " "<<accTime<<" " << numaccs << endl;
	
// 	GA::sync(); // do we really need this
//cout << GA::nodeid() << " Done with the SYNC " << endl;
	return(hvcount);
      }
...
//  GA::sync(); // TODO CHeck if this is needed
  return(hvcount);
}
...
/*& NEw method to return list of interating Is for the current my_rank */

 int PsociGAhamiltonian::fetchMaskList( int my_rank, vector<int> & mask_list )
{
  int me = my_rank; // for MY vector range, what I's do I contribute to 
  int size = GA::nodes();
  ...
  lo[1] = 0;
  hi[1] = 0;
  l_g_mask_number->get( lo, hi, &my_number, &n);

  if ( my_number <= 0 ) return(0);

  mask_list.resize( my_number );
  lo[1] = 0;
  hi[1] = my_number-1;

  l_g_mask->get( lo, hi, &mask_list[0], &n);

  return( mask_list.size());
}


/* New method to acumulate and upload vector mask data to GA */
/* Brute force. Read through the icicol ( filter by cimat as needed). Find what rank should get the Js */
/* Collective operation*/
/* Need to loop over the whole of H BUT do it in parallel*/

//int PsociGAhamiltonian::constructMASKGAAcc(int * v_lo, int * v_hi, GA::GlobalArray * g_v, GA::GlobalArray * g_mask, GA::GlobalArray * g_mask_number)

int PsociGAhamiltonian::constructMASKGAAcc(int * v_lo, int * v_hi, GA::GlobalArray * g_v)
{

// Double check that the vector array have been created

  int return_numi=0; 

  char handleMessage[] = "PsociGAhamiltonian:constructMASKGAAcc:g_v";
  g_v->checkHandle( handleMessage );

  int my_vlow = *v_lo;
  int my_vhi  = *v_hi;
  int me = GA::nodeid(); // for MY vector range, what I's do I contribute to 
  int size = GA::nodes();
...
// allocate space
  int dims[2], chunk[2];
  dims[0] = size; // everyone has a piece but may have NO contributions
  dims[1] = l_maxsef;
  chunk[0] = DISTRIBUTE_EVENLY;
  chunk[1] = l_maxsef;

  char local_title8[] = "g_mask";
  l_g_mask = GA::createGA( C_INT, DATA_NDIM, dims, (char *) local_title8, chunk);
  l_g_mask->zero();

  dims[1] = 1;
  chunk[1] = 1;
  char local_title9[] = "g_mask_number";
  l_g_mask_number = GA::createGA( C_INT, DATA_NDIM, dims, (char *) local_title9, chunk);
  l_g_mask_number->zero();
...
// What piece of H do I have
  ...
  const int buf_index = 0;

      cout << GA::nodeid() << " CONSTRUCT MASK  local range for ME is " <<vlow<<" "<<vhi <<" CI range is " << clow<<" "<<chi<<  endl;
...
//      for(int i=clow; i<=chi; ++i ) { //data parallel across maxsef
        for(int i=0; i<l_maxsef; ++i ) {
         ...
         fetchLocalHblock( i, cimat, icicol, number);
 //        cout << GA::nodeid() <<" next i is " << i << " number is " << number << endl;
         totalelems+=number;

         int start_rng=icicol[0];
         int end_rng=icicol[number-1];

         // Now for this list of Js does my local vector patch potentially overlap ?
         // that is are any of the list of Js within my vector range. If so keep i
         for (int j=0; j< number; ++j ) {
             int jval = icicol[j];
 //            cout << GA::nodeid() <<" test J of " << i << " " << j <<" "<<jval << endl;
             if ( jval <= vhi && jval >= vlow ) {
 //               cout << GA::nodeid() << " Inserting i " << i << "based on a jhi os " << jval << endl; 
                unique_list.insert( i ); // if at least one J then must keep I
                break;
             }
         }
      }

      cout << GA::nodeid() << " CONSTRUCT MASK: found " << unique_list.size() << " % of local list is " << unique_list.size() * 100 / l_maxsef << endl;

/*
      set<int>::iterator it;
      for(it = unique_list.begin(); it != unique_list.end(); ++it ) {
         cout << GA::nodeid() << " values " << *it << endl;
      }
*/


      vector<int> upload(unique_list.begin(), unique_list.end()); 

cout << me << " Size of upload is " << upload.size() << endl;

// Okay Now we need to store into GA MY data
     number = upload.size();
     int lo[2], hi[2], n=1;
     lo[0] = me;
     hi[0] = me;
     lo[1] = 0;
     hi[1] = number-1;
     l_g_mask->put( lo, hi, &upload[0], &n);

cout << me << " put number " << endl;
     lo[1] = 0;
     hi[1] = 0;
     l_g_mask_number->put( lo, hi, &number, &n);


cout <<" PRINT OUT MASK" << endl;

     l_g_mask->print();

     l_g_mask_number->print();



    return_numi = upload.size();
    GA::sync();
    return( return_numi );
}
...
void PsociGAhamiltonian::zeroVector( vector<double> & vector, int number )
{
  const double dZero = 0.0;
  int num = number;
  if ( number > vector.size() ) GA::error(" number > vector.size()",vector.size());
  for(int i=0; i< num; ++i )
    {
      vector[i] = dZero;
    }
...
{
  GA::sync(); // must carve up configuration space pretty evenly

cout << "MAKE CIMAT_LIST" << endl;

  cimat_list.clear();
  int me = GA::nodeid();
  int size = GA::nodes();

/* just ge the LOCAL guys */
...
//not necessary anymore to do this
...
*/
  ...
  int number;
  if ( width != 1 ) GA::error(" width is not 1 ", width);

  for(int i=0; i< width; ++i ) {

 double temp = psociTime();
   number = local_number[ i ];

   if ( number > dataarray.size() ) {
      dataarray.resize( number );
   }

   if ( 2*number  > indexarray.size() ) {
        indexarray.resize( 2*number ) ;
       //cout << GA::nodeid() << " indexarray "<<indexarray.size()<<" "<<offset <<" "<<i<<" "<<j<<" "<<number << endl;
       //GA::error("indexarray failre",1);
   }


// Does going in reverse help ?
...
#ifdef USEMPI
// transpose and convert to fortran indexing
      indexarray[offset] = 1+ivec;
      indexarray[offset+1] = 1+local_icicol[i][j];
#else
      indexarray[offset] = local_icicol[i][j];
      indexarray[offset+1] = ivec; 
#endif

      //cout << "detailed " << i<<" "<<j<<" "<<local_icicol[i][j]<<" "<< offset<<" "<<indexarray[offset+1]<<" "<<indexarray[offset]<<endl;
   }
   prelimTime = psociTime() - temp;
   ...
   temp=psociTime();
#ifdef USEMPI
//"flat" version wnga_gather(a, v, _subs_array, 0, nv);
   pnga_gather(g_v->handle(), (void *)&vectors[index], (int *)&indexarray[0],cflag, number);
#else
   ...
   //getVTime += psociTime() - temp;
   num +=  number;
   fetchTime = psociTime() - temp;
  }

// these are all zero 

/*
cout << "first few values " << number<<endl;
for(int i=0; i< number; i++) {
   cout << "val "<< local_icicol[0][i]<<" "<<vectors[i]<<endl;
}
*/

  cout << GA::nodeid() <<  " Gather times " << num << " " << prelimTime << " " << fetchTime << endl;
   return( num);
}

// only grab an I's woth of elementa instead of an ivec
...
//cout << "allocateLocalScratchMatrixVector size bufV "<<endl;
...
// THis seems horribly slow.
  ...
  accumTime = psociTime() - temp;
#endif

 //cout << GA::nodeid() <<"HVEC put " << assemTime<<" "<<accumTime<<endl;
...
cout << "vacate OVERRIDE bufV2 size to" << endl;
}
...
/* A primitive method that ONLY repackages up data for scatter to nodes. No accumulate is performed
   Zeros are removed
*/
int PsociGAhamiltonian::packDataNoZeros1D(bool transpose, int ivec, int number,  vector<int>& index, vector<double>& buf, vector<int> & indexarray, vector<double> & dataarray ) 
{
    indexarray.clear();
    dataarray.clear();
...
/* populate data array and index array - and remove zeros */
  ...
  double temp = psociTime();
   ...
   for(int i=0; i< combined.size(); ++i ) {
      ...
      if ( row == oldrow ) {
         dataarray[ pushed_index ] += data; // requires presorting
...
int PsociGAhamiltonian::packDataNoZerosAccumulate(bool transpose, int indexnewarray, int ivec, vector<int> & number,  vector<vector<int> >& index, vector<vector<double> >& buf, vector<int> & indexarray, vector<double> & dataarray )
{
    ...
    int itotal = 0;
    int offs = 0;
    int jseftemp=-9, ivectemp=-9;
    //int realbufsize = indexnewarray+1; // calling app starts at zero
    int realbufsize = buf.size(); // the WIDTH of bufV2

    double temp, first=0.0, second=0.0, third=0.0;

//cout << "dataarray size is" << dataarray.size() <<" number is " << number[0] << endl;

// emty contenst into COMBINED THESE would be OLD data from prior is. thatg haven;t yet been dumped to scatter
...
cout << "packDataNoZerosAccumulate: push_back " << ivectemp<<" "<<jseftemp<<" "<<dval << endl;
...
// Now add in the NEW vaklues from the current loop of i
    temp = psociTime();
    ...
    for(int i=0; i< realbufsize; ++i ) {
       ...
       for(int j=0; j< num; ++j ) {
          ++itotal;
          if ( abs(buf[i][j]) > dZero ) {
            //cout << "packZeros " << i<<" "<<j<<" "<<buf[i][j] <<endl;
            combined.push_back( pair<int,double>( index[i][j], buf[i][j] ) );
            buf[i][j] = dZero; // need to do this anyway
          }
       }
...
// at this point everything is in COMUINED
// NOw compress and repopulate dataarray and friends.
...
*/
 ...
 int PsociGAhamiltonian::matrixVectorUpdate(GA::GlobalArray * g_v ,int iroot, int iconf, double Vextra, int * v_lo, int * v_hi,  int & indexnewarray, int nsefi,  vector<int> & bufVnumber, vector<vector<int> >& bufVIndex, vector<vector<double> >& bufV2, 
...
//NOTE: a resize causes a copy to be put on the stack
     int width=nsefi;
     // cout << "on entry nsefi is " << nsefi << " iconf is " << iconf<< endl;
     // cout << "on entry ilo ihi are " << ilo<<" "<<ihi<<endl;
     bufVIndex.resize( width );
     bufV2.resize( width );
     bufVnumber.resize( width,0 );

//change me
     // For now this is REQUIRED
     for(int i=0; i< width ; ++i ) { // need to allocated both dimensions - way too excessive !
       bufV2[i].resize( max(l_maxsparse,l_maxsparse_supp),0.0 );
       bufVIndex[i].resize( max(l_maxsparse,l_maxsparse_supp),0 );
     }

//absolute refs for current iconf

   int isefstart = l_nsef[iconf]; // vector or H indexing ? 
   int isefend = l_nsef[iconf] + nsefi;

/* OKAY now we need to LIMIT the range of J's based on the local width of testVector */
/* We know on entry that iconf is within the local range */
/* how do I know where tio start ? */
// WE can choose to do something different for bufV2

    
// Need to know WHO is doing the last block (final j for the given i )
// based on ilo and ihi (local sefs)


   for(int in=0; in< nsefi; ++in ) {
    // int istart = in + l_nsef[ iconf ]- ilo; // real address for I
     int istart = in + l_nsef[ iconf ]; // real address for I

     int jmyhi = istart; // the end of the H triangle

     int ishift = 1;

     //cout << GA::nodeid() << " ilo ihi istart " << ilo <<" "<<ihi<<" "<<istart << endl;

     if ( jmyhi <= ihi && jmyhi >= ilo ) {
        ishift = 0;
        //cout << GA::nodeid()<<" ishift: I do the last j term " << endl;
     }
...
// this is okay
     for(int j=0; j< numb; ++j ) {
       ...
       if ( jhi >= ilo && jhi <= ihi ) {
          int refjhi = jhi - ilo;
          dtemp += cimat[in][ j ] * testVector[ refjhi ]; //istart offsets over sefs for the given iconf
         // cout << GA::nodeid() << " dtemp " << dtemp <<" numb" << numb << " iconf " << iconf<< " IN  "<<in<<" "<< j<<" jhi "<<jhi<<" refjhi " << refjhi << "ilo ihi are "<<ilo<<" "<<ihi<<" "<< cimat[in][ j ]<<" testV[refjhi] is "<<testVector[refjhi] << endl;

       }
      }

// note numb be is not as useful as it used to be
// Need to see if this is a valid condition
// Probably do not need to SUM here     bufV2[ in ][ numb-1 ] += dtemp;
// After this is a simple PACK procedure which should bre fine withrespect to relative js

// WRONG this presumes the entry was for the i=j term which is no longer true
// It is possible to overwrite the numb-1 entry. Needs ot be fixed

/*
     int insert=numb-1; // works fine if node does all Js only applies to the last block in the new version
     if ( istart    ) {
        insert++; // local Js but not the end: add one  worry about allocated sizes`x
      }
*/

     if ( numb > 0 ) {
        bufVnumber[ in ] = numb+ishift; // how many were found: explain the ishift
        bufV2[ in ][ numb-1 + ishift ] = dtemp;

        //bufVIndex[ in ][ numb-1 ] = icicol[in][numb-1]; // no sum it is always the same
        //bufVIndex[ in ][ istart ] = icicol[in][numb-1]; // no sum it is always the same

        bufVIndex[ in ][ numb-1 + ishift  ] = istart;
        estimate_elements++;
// cout << " ishift " << ishift << " updated dtemp " << istart<<" numb-1 " << numb-1 << " " << icicol[in][numb-1] << " " << dtemp << endl;
    } 
...
//bufVindex might be getting overwritten below
...
// this is okay but wat ch for double counting.
// double testVec = testVector[ istart ]; //need ith coef for the current reduction
// cout << GA::nodeid() << " vpatcvhes " << testVec <<" orig was " << testVector[ istart ] << endl;
// Need to process ALL j's up to the maximj-1 SO we need to detemrine if thius is the contribution to IN
//    for(int j=0; j< numb-1; ++j ) {
//     THis seems to be OKAY
//orig SUMM       bufV2[ in ][ j ] += cimat[in][ j ] * testVec;
     ...
     for(int j=0; j< numb; ++j ) {
       ...
       if ( jhi < istart ) {
       bufV2[ in ][ j ] = cimat[in][ j ] * testVec;
//     cout << GA::nodeid() <<" HV offdiagonals " << istart<<" "<<j << " jhi " << jhi<<" "<<in<<" "<<bufV2[ in ][ j ]<<" "<<testVec<<" "<<cimat[in][ j ]<<endl;

/*
       if ( jhi == 0 && in == 0 ) {
       cout <<" jhi is zero data are " << bufV2[ in ][ j ]<<" "<<j<<" "<<jhi << endl;
       }
*/

       bufVIndex[ in ][ j ] = jhi; // no need to sum
       estimate_elements++;
     }
    // } // numb
       } // jhi istart elimninate the last term
...
/* how do I know where tio start ? */
   ...
   for(int in=0; in< nsefi; ++in ) {
     ...
     for(int j=0; j< numb; ++j ) {
       ...
       cout << " INNER PROD iconf " << iconf <<" J loop " << jhi << endl;
          dtemp += cimat[in][ j ] * testVector[ jhi ]; //istart offsets over sefs for the given iconf
          cout << "dtemp " << dtemp <<" "<<in<<" "<< j<<" jhi "<<jhi<<" "<< cimat[in][ j ]<<" "<<testVector[ jhi] << endl;
     }

     bufVnumber[ in ] = numb; // how many were found
     bufV2[ in ][ numb-1 ] += dtemp;
     bufVIndex[ in ][ numb-1 ] = icicol[in][numb-1]; // no sum it is always the same
...
// packDataNoZerosAccumulate empties dataarray and indexarray internaly
   indexarray.clear(); // excessive
   dataarray.clear();
//cout << "befor pack " << indexnewarray << " " << iroot<<" "<< bufV2.size() << endl;
...
//cout << "SCatter:pack zeros " << newrealnumber << endl;
...
#endif
#ifdef SCATFLAT
     //cout << "scatte rdata " << dataarray.size() << endl;
     //cout <<" Data to dump " << endl;
     //for (int k=0; k< newrealnumber; k++ ){
     //    cout << dataarray[k]<<" "<<indexarray[2*k]<< " "<<indexarray[k*2+1] <<endl;
    // }
...
//bufV2is a global so this should be okay
