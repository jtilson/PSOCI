...
int PsociGAbasis::computeMASK()
{
    //int value = constructMASKGAAcc(int * v_lo, int * v_hi, GA::GlobalArray * g_v);
    int value = local_hamiltonian->constructMASKGAAcc(&vector_lo[0], &vector_hi[0], local_ga);
    return ( value );
}
...
//if ( GA::nodeid() == 0 ) cout << " December EXPERIMENTAL gather/scatter " << endl;
...
// vcount = local_hamiltonian->matrixVectorProductsIncoreGatherScatterNoVectorGetDecember( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );

cout << "March version " << endl;
vcount = local_hamiltonian->matrixVectorProductsIncoreGatherScatterLocalVectorGetMarch( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );

//if ( GA::nodeid() == 0 ) cout << " OLD LOWLEVEL methods " << endl;
//vcount = local_hamiltonian->matrixVectorProductsSplitChunkScatterAccumulateLowLevelAccumulate( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );

/* currently working 
if ( GA::nodeid() == 0 ) cout << "NEW gather V gaPUT hv" << endl;
vcount = local_hamiltonian->matrixVectorProductsSplitAccumChunkGatherVector( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );
*/
...
if ( GA::nodeid() == 0 ) cout << "DIRECT FETCH MATRIXPRODUCTS non scatter NEW gather V gaPUT hv" << endl;
vcount=local_hamiltonian->matrixVectorProductsSplitChunkScatterAccumulateLowLevelAccumulateDirect( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );
// vcount = local_hamiltonian->matrixVectorProductsSplitChunkScatterAccumulateLowLevelAccumulateDirectFetch( start, end, local_ga, &vector_dims[0], local_Hga, &vector_dims[0],&vector_lo[0], &vector_hi[0]  );
...
// cout <<"PsociGABasis: allocateScratchVectorSpaceScatterAccumulate" << endl;

     local_hamiltonian->allocateLocalScratchMatrixVector();
     if ( GA::nodeid() == 0 ) cout << "NO skip[ SKIP resizeBuffer step " << endl;
