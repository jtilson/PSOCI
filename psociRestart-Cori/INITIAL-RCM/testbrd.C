...
// DO the brdcst a number of times and average results
   ...
   int testrun = 1;
   double tempLoop;
   vector<double> brdvector2;
   while ( runsize > 1 && testrun < MAXTEST ) {
    brdvector2.resize( TESTSIZE);
    if (rank == 0 ) {
       for(int i=0; i< TESTSIZE; ++i ) {
       brdvector2[i] = -2;
       }
   ...
   for (int itest=0; itest<NUMTEST; ++itest) {
       ++dcount;
       MPI_Bcast( &brdvector2[0], runsize, MPI_DOUBLE, 0, MPI_COMM_WORLD );
   }
   ++testrun;
   double loopTime = (  MPI_Wtime() - tempLoop ) / dcount;
   GCOUT << "TEST2-LOOP is " << testrun << " Me is " << rank << " SIZE (W) " << runsize << " Bcast time (s)(ave) " << loopTime << endl;
   brdvector2.clear();
   }
