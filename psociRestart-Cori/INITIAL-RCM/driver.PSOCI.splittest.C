...
#ifdef USEMPI
  GCOUT << "Using parallel MPI Allreduce for H*V updates in scatter method" << endl;
#else
  GCOUT << "Using GA dgop for  H*V updates in scatter method" << endl;
#endif
#endif
...
// Perform a restart like configs read
  ...
  const int local_maxspatials = configs.numTotalSpatials();
...
#ifdef STATICEVEN
  GCOUT <<"STATICEVEN loopping used IF scatter is selected " << endl;
#endif
...
// for big hopper run use this: numIOservers == GA::nodes() / 12;
   ...
   if ( numIOservers == GA::nodes() && GA::nodes() >= 12 ) {
       numIOservers == GA::nodes() / 12; // Im TACC and cluster nodes don;t work here
        if ( GA::nodeid() == 0 ) cout << "cluster nodes don't work: divide by 12 " << endl;
   }
...
// dump files into a subdir. we want numsubdirs << numcores and deterministic
   ...
   int numsubdirs = numfiles * g_size / maxfilesperdir;  // numdirs based on files
...
// Can we split differnetly so that all sefs for a given icionf belong on a node?
//We need to do an irregular crate here




#ifdef NEWVECTORDISTRIBUTION

 cout << "OPEN UP NEW VCTOR SPACE "<<endl;
 int nblocks[2];
 //nblocks[0]=local_maxspatials ; // maxsef dim broken into maxspatials blocks

 nblocks[0]=g_size; // an M*1 processor grid
 nblocks[1]= 1; //  spread evenly accross vertical strips

//map is simply l_nsef
//int map={0,2,6,0,5};
//we need to do our own distribution

  vector<int> map;

  int stride = l_nsef.size() / g_size;

  cout << "stride is " << stride <<" gsize is " << g_size<< endl;

/*
  for(int it=0; it< l_nsef.size(); it += stride) {
     map.push_back( l_nsef[it]);
  }
*/

  int index=0;
  for(int it=0; it< g_size; it++)   {
     map.push_back( l_nsef[index] );
     index += stride;
  }
  ...
  if ( map.size() < GA::nodes() ) GA::error(" map size smaller than partition: Use fewer nodes ", 1);
  
  g_v  = GA::createGA( C_DBL, vdim, dims, (char *) vector_title, nblocks, &map[0]);
  g_v->zero();

  cout << "next space " << endl;

  g_Hv = GA::createGA( C_DBL, vdim, dims, (char *) Hvector_title, nblocks, &map[0]);
  g_Hv->zero();

  cout << "done with vector open " << endl;
...
#ifdef NEWVECTORDISTRIBUTION
  cout << "Rnorm modified distribution " << endl;
  g_rnorms = GA::createGA( g_v, (char *) rnorm_title);
#else
...
// MAybe we need to relax this constraint ?
  if ( scatter_products ) {
...
// Reset ( if necc) the value of vector_chunk
  ...
  const int maxmempercore = 512; // use 1024 for Edison
...
// Generate vector MASK
   ...
   GCOUT << "SKIP computing MASK" << endl;
   //GCOUT << "Generate mask " << endl;
   //vector_set.computeMASK();
...
*/
    ...
    if ( showevals && GA::nodeid() == 0 ) {
     cout << "Current vector under optimization " << curroot<<" "<<rnorms[curroot-1]<<endl;
      ...
      cout << "Curreno solution timers report " << diagTimes<<" "<<" "<<genTimes<<" " <<rnormTimes<<" "<<expandTimes<<" "<<appendTimes << " " << precondTimes << endl;
    }
