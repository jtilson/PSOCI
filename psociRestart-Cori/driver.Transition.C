  ...
  while (1)
    {
      static struct option long_options[] =
        {
          ...
          {"configs2_filename (c)",   required_argument, 0, 'T'},
      ...
      int option_index = 0;
      c = getopt_long (argc, argv, "R:Wn:s:dm:c:g:v:ri:a:I:C:F:N:hHVSu:p:T:",
      ...
      switch (c)
        ...
        case 'd':
          ...
          cout << "configs2_filename \t " << configs2_filename << endl;
	  ...
	  cout << "maxsparse \t\t " << maxsparse << endl;
        ...
        case 'h':
          ...
          cout << "--configs2_filename \t\t <string> filename of second input configuration file " << endl;
...
int main(int argc, char **argv)
{
  ...
  GCOUT << "configs2_filename \t" << configs2_filename << endl;
...
// Get both sets of configs - transition moment
  ...
  PsociConfigs configs2( configs2_filename ); //File is opened/closed at the read
  long configBytes2 = driverFetchConfigurations( configs2 );
  GCOUT << "Current per-core Configs2 memory (MB) " << configBytes2/(1024*1024) << endl;
  GCOUT << endl;
...
// Need to fetch a subset of configs, create determinants and push those dets to GA space
  ...
  GCOUT << "Build distributed Transition moment GA_DET space " << endl;
...
*/
...
#ifdef DIRECT
  PsociGADeterminants deters_det(configs,  g_det, g_det_num, g_nsef );
  ...
  deters_det.generateDistributedDeterminantsSmallMemoryGA( configs, gendettime );
#else
...
//Perform for the second set of configs

  GA::GlobalArray * g_det2; // declare global det data 
  GA::GlobalArray * g_det_num2;
  GA::GlobalArray * g_nsef2;

  pair<int, double> gendettime2;
#ifdef DIRECT
  PsociGADeterminants deters_det2(configs2,  g_det2, g_det_num2, g_nsef2 );
  deters_det2.generateDistributedDeterminants(); // One needs to delete configs 
#else
   PsociGADeterminants deters_det2( g_det2, g_det_num2, g_nsef2 );
#ifdef SINGLENODECONFIGS
  GCOUT << "Build determinates with single core configs method: small memory footprint" << endl;
  deters_det2.generateDistributedDeterminantsSmallMemoryGA( configs2, gendettime2 );
#else
  deters_det2.generateDistributedDeterminants( configs2 );
#endif
#endif

#ifdef SINGLENODECONFIGS
  GCOUT << "Time for distributed second determinant computations: me is " << gendettime2.first<<" time " << gendettime2.second << endl;
#endif
  vector<int> l_nsef2;
  deters_det2.assembleAndBrdcstNsef( l_nsef2 ); //don't need l_nsef anymore

  int numBasisFtns2 = deters_det2.fetchNumBasisFunctions(); // Need this for CIDEN work

// Compare them

  if ( numBasisFtns != numBasisFtns2 ) {
     GA::error(" numBasisFtns != numBasisFtns2 ",1);
  }
...
///////////////////////////////////////////////
/* Open up set of moints file, read it and brdcst all ints 
   ...
   Both configs sets must be based on this same set of MOs
*/ 
...
// WHich configs list is biggest?
   int maxsef1= deters_det.fetchGlobalMaxSefs(); // current problem size
   int maxsef2= deters_det2.fetchGlobalMaxSefs(); // current problem size

   int maxsef = max(maxsef1,maxsef2);
...
*/
   ...
   */  
...
#ifdef SUPPLEMENTAL
   int maxwidth = 1 + ( percent_maxwidth * (maxsef  / 100L) ) ; // round upP - hack over the fact that 30*maxsef > 2GB
   ...
   int retvalue;
#ifdef SUPPLEMENTAL
     retvalue = SpecifyDRA(construct_hamiltonian, d_cimat,d_icicol,d_number,d_diag_sef, d_cimat_supp, d_icicol_supp, d_number_supp, readSparse, readSparse_supp, readWidth );
#else
 ...
 PsociGAhamiltonian hamilton(maxsparse, maxsef, supp_maxsparse, maxwidth, &deters_det, &deters_det2, &mos );
 ...
 int nsize = hamilton.computeNsefEntryPoints( l_nsef ); // I _think_ this only aplies to the LHS (and thus configs1)
...
*/
 ...
 if ( construct_hamiltonian == CONSTRUCT || construct_hamiltonian == NONE ) {
   ...
   GCOUT << "Construct Experimental 2D TWOWAY distributed transition hamiltonian " << endl;
   long numH = hamilton.constructGlobalTransitionHamiltonian2WAY(Hinfo);
...
/* At this point we want to dump ALL NO sets to individual files using the COlumbus library mowrit. Now, 
...
/* Now we want to save NOs to disk */
