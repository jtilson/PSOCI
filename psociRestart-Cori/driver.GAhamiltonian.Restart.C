...
// Need to fetch a subset of configs, create determinants and push those dets to GA space
  ...
  GCOUT << "Before nsef " << deters_det.fetchGlobalMaxSefs() << endl;
  vector<int> l_nsef;
  deters_det.assembleAndBrdcstNsef( l_nsef ); //don't need l_nsef anymore
  GCOUT << "after nsef " << deters_det.fetchGlobalMaxSefs() << endl;
...
#ifdef SUPPLEMENTAL
/* Do not use this new DRA code doesn't use this
   If you don't want to use DRA then you can use SUPPLEMENTAL
*/
 ...
 GCOUT << "Setting GAH sparsity to " << maxsparse << endl;
 GCOUT << "maxsef is " << maxsef << endl;
 GA::printStats();
...
/* This now not used anymore  applies only to the fetch2D code
...
*/
 ...
 GCOUT  << "Closing up DRA as we have our data now " << endl;
...
*/
 ...
 int numSoughtRoots = 40; // more like 50 to 100 int maxsubspace = 4 * numSoughtRoots; // 4 expansions per eigenvector (
...
// Maximum number of iterations in the solution 
  int max_iterations_per_rtol = 1000; 
...
// On Hopper we noticed these are not zero on construction
 g_v->zero();
 g_Hv->zero();
...
*/
 ...
 // Try to read them back in and see if the diagonal with augmentation stuff works.
 
//TODO implement a vector RESTART that needs this
/*
 gavectors.openVectorRead( GA::nodeid(), vecname );
 GCOUT << gavectors.getNumberVectors();
 GCOUT << gavectors.getNumberVectorsSought();
...
// IMPORTANT: This "setpresent" call is here because we PRECOMPUTED some terms as partof toolkit testing: NOT DOING THIS ADDS A FW h*V TO THE CALC
  ...
  int curroot=1; //Assume optimize from the first root
...
// Not wanting this for all systems and especially not for production runs
/*
    if ( GA::nodeid() == 0 ) {
...
//TODO specvify HOW MANY to save to disk
   string vecname="vector_test.txt";
   gavectors.openVectorWrite( GA::nodeid(), vecname );
