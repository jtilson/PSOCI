...
void InitializeDRAservice( int g_rank, PsociDRAservices & DRAservice, int IOservers )
{
  ...
  DRAservice.reportDRAParams();
#if 0
  cout << "Got the DRA initialized -2 " << g_rank << endl;
#endif
}
...
void FetchICICOLFromDRA( HSOURCE state,  PsociDRArestart & d_icicol,
                              PsociDRArestart & d_icicol_supp, 
                              PsociGAhamiltonian & hamilton, pair<int,double> & info )
{
  int g_rank = GA::nodeid();
  double timein = psociTime();
  FetchICICOLFromDRA( state, d_icicol, d_icicol_supp, hamilton );
  info.second = psociTime() - timein;
  info.first = g_rank;
}
...
{
  if ( state != RESTART ) {
    cerr << "Erroneous call to Split version FetchICICOLToDRA " << endl;
    GA::error( "Erroneous call to Split version of FetchICICOLToDRA ", state);
  }
  ...
  d_icicol_supp.waitDRA();
  cout << "Vacating FetchICICOLFromDRA" << endl;
}
