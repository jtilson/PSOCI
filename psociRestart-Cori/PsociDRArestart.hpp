...
**************************************************************************************/
/**
 *   @file PsociDRArestart.hpp
 *
 */

#ifndef PSOCIDRARESTART_H
#define PSOCIDRARESTART_H
...
const int MAX_DRA_DIMS = 5;
const int DEF_DRA_MODE = DRA_RW;
const int DEF_DRA_TYPE = MT_DBL;

#define DRA_TRUE (logical)1
#define DRA_FALSE (logical)0

class PsociDRArestart {
...
public:
  PsociDRArestart();
  PsociDRArestart( int rank, string arrayname, string filename );
  void reportName();
  void reportFilename();
