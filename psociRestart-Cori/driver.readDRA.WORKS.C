...
// Create a 2dim Global array of testData3: (nbasis x nroots)
...
// Test dumping GLobal Array test data to PsociVector disk.
  ...
  PsociGAvectors resVec( info2.second, g_vector );
...
// Test reading GA from PsociVector info g_read_vector
  ...
  PsociGAvectors resVec2( info2.second, g_read_vector );
...
//For now dump the entire GA into the first nroots oclumns of DRA
  ...
  if ( dravector.dumpGAtoDRA( g_vector) != 0 ) {
     cout << "Wholesale Dump failed " << endl;
  }
