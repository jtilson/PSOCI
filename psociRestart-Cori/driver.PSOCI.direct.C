  ...
  while (1)
    {
      ...
      int option_index = 0;
      c = getopt_long (argc, argv, "P:R:Wn:s:dm:c:g:v:ri:a:I:C:F:N:hHVSu:p:",
...
// Need to fetch a subset of configs, create determinants and push those dets to GA space
  ...
  if (hamiltonian_store) {
    GA::error("PSOCI cannot attempt a store of the DRA files ",1);
    construct_hamiltonian = CONSTRUCT;
  }

  if (hamiltonian_restart) {
    GA::error( "PSOCI cannot attempt a restart from the DRA files ",1);
...
*/
   ...
   // NO DRA required for direct matrix-product methods

   int maxwidth=0;
   string testdir="direct";
   PsociGAhamiltonian hamilton(testdir, maxsparse, maxsef, supp_maxsparse, maxwidth, &deters_det, &mos );

   GCOUT << "direct M*V: Setting NXTVAL CHUNK to " << nxtval_chunk << endl;
   hamilton.setNxtvalChunk( nxtval_chunk );
 
 // Extra bit needed by all but data comes from Det processing
...
// Compute H ONLY DIAG elements are required
...
*/
   ...
   hamilton.computeOnlyDiagElements();
...
// we can delete H at this point
  ...
  GCOUT << " pre-generate stagger chunk list " << endl;
...
// NEED real diags to use this
    if ( numNOroots != 0 ) GA::error(" analyze disabled for now",1);
...
#ifdef DIRECT
   GA::error("DIRECTMATRIXPRODUCTS best used WITHOUT -DDIRECT",1);
#endif
