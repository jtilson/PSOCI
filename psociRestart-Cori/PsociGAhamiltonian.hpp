...
int matrixVectorProductsSplitChunkMultipleJchunks( int start, int end,  GA::GlobalArray * g_v, int * v_dims, GA::GlobalArray * g_Hv, int * hv_dims, int * v_lo, int * v_hi );
 ...
 int fetchNchunk(); 
 int generateNodeRanking();
 int generateNodeRankingInMemory();
 void fetchGArowFulllength(vector<int> & row,  int irow );
 void fetchLocalIcicolGArows(vector<int> & loopindex, set<int> & totalinodes,  vector<set<int> > & icimat );

 void constructGlobalHamiltonianAltOrbMapChunkJmapEstimatesOnly();
};
