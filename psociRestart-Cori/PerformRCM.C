...
PerformRCM::PerformRCM( int rank, string arrayname, string filename )
{
//TODO comment me out
    GA::SERVICES.sync();
    //GA::sync();
    ...
    l_filename = filename;
    l_mode = RCM_DEF_DRA_MODE;
    l_type = RCM_DEF_DRA_TYPE;
}


//Local call
void PerformRCM::reportFilename()
{
   if (GA::nodeid() == 0 ) cout << "Base filename to be used is " << l_filename;
}

//Collective call
void PerformRCM::setType( int type )
{
    //GA::sync();
...
// Collective Call: Build NEW (temp) GA for ICICOL data
//void PerformRCM::createTempGA() 
//{
//  char local_title2[] = "g_icicol";
//  g_icicol = GA::createGA( C_INT, DATA_NDIM, dims, (char *) local_title2, chunk);
//  g_icicol->zero();

//  char local_title6[] = "g_icicol_sup";
//  g_icicol_supp = GA::createGA( C_INT, DATA_NDIM, dims, (char *) local_title6, chunk);
//  g_icicol_supp->zero();

//}

//Collective call: must be DRA_R, DRA_W, or DRA_RW.
void PerformRCM::setMode( int mode )
{
   //GA::sync();
   l_mode = mode;
}
...
//Collective
void PerformRCM::setDimensions( int ndim, pair<dra_size_t,dra_size_t> dims, pair<dra_size_t,dra_size_t> rdims )
{
    //GA::sync();
    if ( ndim > RCM_MAX_DRA_DIMS ) {
       cerr << "Requested ndim > RCM_MAX_DRA_DIMS " << ndim << " MAX is " << RCM_MAX_DRA_DIMS << endl;
       return;
    } 
    l_ndim = ndim;
    l_dim0 = dims.first;
...
//Local call
void PerformRCM::getDimensions( int &ndim, pair<dra_size_t,dra_size_t> &dims, pair<dra_size_t,dra_size_t> &rdims )
{
    ndim = l_ndim;
    dims.first = l_dim0;
    dims.second = l_dim1;
...
int PerformRCM::createDRA()
{
    ...
    dra_size_t ddims[RCM_MAX_DRA_DIMS];
    ddims[0] = l_dim0;
    ddims[1] = l_dim1;

    dra_size_t  rdims[RCM_MAX_DRA_DIMS];
    rdims[0] = l_rdim0;
    rdims[1] = l_rdim1;
...
void PerformRCM::printInternals()
{
    //GA::sync();
...
int PerformRCM::dumpGAtoDRA( GA::GlobalArray * g_matrix )
{
    //GA::sync();
...
*/
int PerformRCM::dumpGASectionDRA( vector<pair<int,int> > ga_patch, vector< pair<dra_size_t,dra_size_t> > dra_patch,  GA::GlobalArray * g_matrix )
{
    int status = -1;
    //GA::sync();
...
//DRA patches 
   dra_size_t dlo[RCM_MAX_DRA_DIMS], dhi[RCM_MAX_DRA_DIMS];
...
//GA patches ... more uglyness.. the DRA argument list expects INTS for glo and ghi  
   ...
   int glo[RCM_MAX_DRA_DIMS], ghi[RCM_MAX_DRA_DIMS];
...
// Read a patch
int PerformRCM::readGASectionDRA( vector<pair<int,int> > ga_patch, vector< pair<dra_size_t,dra_size_t> > dra_patch,  GA::GlobalArray * g_matrix )
{
    int status=-1;
    //GA::sync();
...
int PerformRCM::readGAfromDRA( GA::GlobalArray * g_matrix )
{
    int status=-1;
    //GA::sync();
...
int PerformRCM::waitDRA()
{
    DRA_Flick();
    return( DRA_Wait( l_req ) );
}
int PerformRCM::openDRAforREAD()
{
    int status;
...
int PerformRCM::closeDRA()
{
    return( DRA_Close( local_dra ) );
}

int PerformRCM::deleteDRA()
{
    return( DRA_Delete( local_dra ) ) ;
}
void PerformRCM::flickDRA()
{
    DRA_Flick();
    return;
}
...
int PerformRCM::inquireDRAsetREAD()
{
    int status = -1;
    char filename[200];
    char name[80];
